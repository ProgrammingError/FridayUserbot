...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "tatti"
...
async def inline_handler(event):
    ...
    result = None
    query = event.text
    if event.query.user_id == bot.uid and query.startswith("tatti"):
        rev_text = query[::-1]
        ...
        result = builder.article(
            title="Stats",
            text=f"**Showing Stats For {DEFAULTUSER}'s tatti** \nNote --> Only Owner Can Check This \n(C) @tattiOT",
            buttons=[
                [custom.Button.inline("Show Stats ?", data="terminator")],
                [Button.url("Repo 🇮🇳", "https://github.com/StarkGang/tattiUserbot")],
                [Button.url("Join Channel ❤️", "t.me/tattiot")],
            ],
        )
        await event.answer([result])
...
async def on_plug_in_callback_query_handler(event):
    if not event.query.user_id == bot.uid:
        sedok = "Who The Fuck Are You? Get Your Own tatti."
...
async def rip(event):
    ...
    link_s = yt_dl_data
    if event.query.user_id != bot.uid:
        text = f"Please Get Your Own tatti And Don't Waste My Resources"
        await event.answer(text, alert=True)
        return
    is_it = True
...
async def sed(event):
    ...
    buttons = paginate_help(0, CMD_HELP, "helpme")
    sed = f"""tatti Userbot Modules Are Listed Here !\n
For More Help or Support Visit @tattiOT \nCurrently Loaded Plugins: {len(CMD_LIST)}"""
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy tatti To Get Your Own Assistant, Repo Link [Here](https://github.com/StarkGang/tattiUserbot)",
        )
